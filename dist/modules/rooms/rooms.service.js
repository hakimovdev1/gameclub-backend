...
let RoomsService = class RoomsService {
    ...
    async create(dto) {
        if (await this.rooms.exists({ where: { name: dto.name } })) {
            throw new domain_exception_1.ConflictException('ROOM_NAME_TAKEN', 'A room with this name already exists');
        }
    ...
    async findById(id) {
        ...
        if (!room) {
            throw new domain_exception_1.ResourceNotFoundException('Room', id);
        }
        return room;
    }
    async update(id, dto) {
